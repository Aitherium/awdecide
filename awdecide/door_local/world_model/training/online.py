...
World-model program slice 7. CLEAN-ROOM implementation: the *concepts* —
surprise-gated plasticity (learn faster when the world violates prediction)
and a homeostatic set-point (a slow negative-feedback loop holding an
activity statistic inside a band) — come from a restricted-license research
project sandboxed under .RESEARCH/; concept provenance and the license
boundary are documented in REFERENCE-homeostatic-lr.md next to this file
(the project is deliberately not NAMED here: check_research_boundary.py
hunts its identifiers in shipped code, and a citation is indistinguishable
from a derivation to a scanner — the .md carries the credit). The mechanism
is written against LeWM's own quantities: train_step's latent MSE as the
surprise signal, per-dim latent variance as the homeostatic statistic, and
sig_lambda (SIGReg weight) as the effector.
