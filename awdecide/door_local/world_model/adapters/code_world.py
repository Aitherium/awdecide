...
The adapter observes landmarks (from prospector) and code chunks (from codegraph),
encodes them into a fixed 768-dim embedding, and exposes a vocabulary of focus
actions (e.g. "focus on landmark X", "traverse to chunk Y").

Step execution is a loud NotImplementedError: code-world stepping (editing, running
code) arrives with the ADK sandbox pack; this adapter is observe/rank-only today.

CONTRACT:
  Conforms to world_model.contracts.EnvironmentAdapter — no AitherOS imports, stdlib
  + optional numpy only.
