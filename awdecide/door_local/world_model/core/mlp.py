...
Extracted from AitherOS lib/cognitive/LearnedWorldModel.py (source of truth).
This module provides the core MLP transition prediction network without
AitherOS-specific dependencies. Drift between this and LearnedWorldModel.py
is checked by test_mlp_parity.py.
...
class MLPWorldModel:
    ...
    Extracted from AitherOS lib/cognitive/LearnedWorldModel.py (source of
    truth). Provides neural transition prediction for unseen (state, action)
    pairs with a tabular fallback. Mode state machine: tabular → hybrid →
    neural.
