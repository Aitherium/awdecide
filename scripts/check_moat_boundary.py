...
Rules:
  * **MOAT001** no monorepo import (`lib.`, `services.`, `from AitherOS`). awdecide
    has ZERO runtime dependencies and is standalone by contract; one of these is a
    `ModuleNotFoundError` on a stranger's machine, and the duck-typed backend seam
    exists precisely so richer integrations attach from OUTSIDE the published
    package.
...
archive) — never 0 for "I could not look".
    ...
    python scripts/check_moat_boundary.py [dist/awdecide-*.whl dist/*.tar.gz ...]
...
#: scans. `__init__` is the public surface, `contract` the typed question/answer,
#: `loop` the decide-act-resolve cycle, `ledger` the Brier record they are scored
#: against. Drop any one and the package installs fine and cannot do its job.
_KEYSTONES = ("awdecide/__init__.py", "awdecide/contract.py",
              "awdecide/loop.py", "awdecide/ledger.py")
...
def self_test() -> int:
    ...
    def wheel(members: dict) -> Path:
        p = Path(tempfile.mkdtemp()) / "awdecide-0.0.0-py3-none-any.whl"
    ...
    check("MOAT001 catches a monorepo import",
          any(f.startswith("MOAT001") for f in
              inspect(wheel({**clean, "awdecide/x.py": "from lib.core import X\n"}))), True)

    check("MOAT002 catches a debt id",
          any("debt-ledger row id" in f for f in
              inspect(wheel({**clean, "awdecide/x.py": "# see D-0000\n"}))), True)

    check("MOAT002 catches a checker rule id",
          any("checker rule id" in f for f in
              inspect(wheel({**clean, "awdecide/x.py": "# MOAT001 says so\n"}))), True)
    ...
    check("MOAT003 refuses an artifact missing a keystone",
          any(f.startswith("MOAT003") for f in
              inspect(wheel({"awdecide/cli.py": "x=1\n"}))), True)

    # The package's own imports should not be flagged.
    check("does NOT flag the package's own imports",
          inspect(wheel({**clean, "awdecide/y.py":
                         "from awdecide.contract import Question\n"})) == [],
          True)
    ...
    try:
        broken = Path(tempfile.mkdtemp()) / "awdecide-0.0.0.tar.gz"
